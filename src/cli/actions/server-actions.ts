...
export class ServerActions {
  ...
  async showInfo() {
    ...
    if (impl) {
      ...
      this.logger.print(
        `    Client capabilities: ${JSON.stringify(currentSession.server.server.getClientCapabilities())}`,
      );
    } else {
      this.logger.print('No client info found');
    }
    this.logger.print(`    Client sessionId: ${currentSession.server.server.transport?.sessionId}`);
    this.logger.flushPrint();
  }
  ...
  async listTools() {
    ...
    Object.entries(tools || {}).forEach(([name, tool]) => {
      this.logger.print(`    ${name}${tool.description ? ': ' + tool.description : ''}`);
    });
    this.logger.flushPrint();
  }
  ...
  async listResources() {
    ...
    const resources = (currentSession.server as any)['_registeredResources'] as Record<
      string,
      { name: string; metadata?: { description?: string } }
    >;
    Object.entries(resources || {}).forEach(([uri, resource]) => {
      this.logger.print(
        `    ${resource.name}${resource.metadata?.description ? ': ' + resource.metadata.description : ''}`,
      );
    });
    this.logger.flushPrint();
  }
  ...
  async addResource(name: string) {
    ...
    this.logger.info(`Resource '${name}' added from config and clients notified`);
  }
  ...
  async listPrompts() {
    ...
    const prompts = (currentSession.server as any)['_registeredPrompts'] as Record<
      string,
      { description?: string }
    >;
    Object.entries(prompts || {}).forEach(([name, prompt]) => {
      this.logger.print(`    ${name}${prompt.description ? ': ' + prompt.description : ''}`);
    });
    this.logger.flushPrint();
  }
